...
The process of creating one online-edition issue from a corresponding print-edition of _Rootstalk_ can be summarized as follows:
  ...
  - In your working directory, create a new [YAML text file](https://yaml.org/) using the text editor of your choice. The file name must be of the form ***year-term***`.yml`.  For example, the `2016-spring` issue must have a corresponding YAML file named `2016-spring.yml`. The format/content of this YAML file is detailed in the next section.
...
The supplemental [Rootstalk's Git Workflow in Windows](./Rootstalk-Git-Workflow-in-Windows.md) document includes a detailed description of the `Git` workflow in a &copy;Windows desktop environment.

# Useful Links

| Purpose | Link |
| ---     | ---  |
| _Rootstalk_ in production, a _Hugo_ static site hosted on _Azure_ | https://rootstalk.grinnell.edu |
| Senay Gokcebel's development instance of the _Rootstalk_ site hosted on _Azure_ | https://delightful-stone-01bd98310.azurestaticapps.net |
