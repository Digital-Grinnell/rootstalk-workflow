...
_Rootstalk: a Prairie Journal of Science, Culture and the Arts_ is a print and online, multi-media, interdisciplinary journal published in conjunction with the Center for Prairie Studies at Grinnell College.  The online edition is published as part of _Digital.Grinnell_ at [https://rootstalk.grinnell.edu](https://rootstalk.grinnell.edu).  The online edition of _Rootstalk_ is generated from content published in a corresponding print edition.  Students, alumni, staff, and faculty of the College have all been involved in creating _Rootstalk_.
...
Each publication run of _Rootstalk_ is an `issue` consisting of several `articles` and `extras`.  Each article consists of text and media (photos, illustrations, poetry, audio, video, etc.) authored by one or more named individuals, typically presented on one or more contiguous pages.  Extras often include images without significant text that are distributed throughout each issue.  Issues are created in corresponding academic fall or spring terms.  The issues created in a single academic year make up one `volume`, expressed as a roman numeral.
...
## The Print Edition
The print-edition of each _Rootstalk_ issue is created using [Adobe InDesign](https://www.adobe.com/products/indesign.html).  _InDesign_ output comes in two forms, an `.indd` file where the content and layout are stored for use exclusively within _InDesign_, and one or more `.pdf` files used to generate printed, paper copies.
...
The online-edition of each _Rootstalk_ issue is published using [Hugo](https://gohugo.io) as part of the [Rootstalk website](https://rootstalk.grinnell.edu).  Each online issue is composed of several articles, and each article is represented by a single [Markdown](https://en.wikipedia.org/wiki/Markdown) file coupled with media files housed in one of three web-accessible cloud storage services including [Azure BLOB Storage](https://azure.microsoft.com/en-us/services/storage/blobs/).
...
The process of creating one online-edition issue from a corresponding print-edition of _Rootstalk_ can be summarized as follows:
  ...
  - Save a local copy of the issue's _InDesign_ `.indd` file on your hard disk. Rename it to be of the form `year-term.indd` if necessary.  For example, the `2016-spring` issue's `.indd` file must be named `2016-spring.indd`. 
  - Open the `.indd` file in _InDesign_ and from the menu choose `File` | `Export...` | `HTML` and all default options to export the issue to HTML format.
